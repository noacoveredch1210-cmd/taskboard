...
### 複数の書き込みを 1 トランザクションにまとめる

並び順の再採番、ボードの作成、ボードの編集は、いずれも複数の書き込みに分かれる。これをクライアントから個別のリクエストで投げると、途中で失敗したときに「列は消えたのにタスクの退避は済んでいない」「オーナーが登録されず誰も入れないボード」といった中途半端な状態がサーバーに残る。しかも失敗時の巻き戻しは再取得なので、壊れた状態を取り直して「復旧した」ことにしてしまう。

そこでクライアントは値ではなく**どうしたいか**（「このタスクを A と B の間へ」「列はこの並びであるべき」）だけを送り、サーバーが 1 トランザクションで適用する設計にした。並び順の採番も再採番もサーバーが持つ。
...
作成（`POST /boards`）も同じ形にしてある。ボード本体・作成者のオーナー登録・最初の列を 1 トランザクションで作る。分けて書くと、オーナー登録が抜ければ**誰も入れないボード**、列が抜ければ**列の足りないボード**ができ、どちらも利用者からは直しようがない。統合テストでは、列の作成をわざと失敗させて**ボードごと残らない**ことを確認している。

この結果、`order_index`・ボード作成・ボード編集・一覧取得のすべてで「複数の書き込みは 1 トランザクションにまとめ、クライアントは意図だけを送る」形に揃った。
...
# サーバー統合: Testcontainers + PostgreSQL (43 tests)
...
- **SQL と所有権チェック** → 実 PostgreSQL を立てる統合テスト（Testcontainers）

## 開発者向け

ローカルでの起動手順と API の一覧は [docs/development.md](docs/development.md) にまとめている。
