...
- **ログの外部集約先が未設定**。構造化ログ（JSON）は出しているが、送り先は標準出力のみで、Seq や CloudWatch のような集約基盤には繋いでいない。Render のログ画面で見る前提。
