...
| GET | `/api/boards` | 参加しているボード一覧（自分の役割付き） |
...
| PUT | `/api/{positions\|tasks\|categories}/{id}` | 更新（ボードの更新はオーナーのみ） |
| PUT | `/api/boards/{id}` | ボード更新（オーナーのみ） |
| DELETE | `/api/{boards\|positions\|tasks\|categories}/{id}` | 削除（ボード・列はオーナーのみ） |
...
| DELETE | `/api/boards/{id}/members/{userId}` | メンバーを外す（オーナー / 本人） |
...
- **共有ボードの同時編集は last-write-wins**。楽観的更新は「自分のリクエストが失敗したとき」の巻き戻し（再取得 → スナップショット）は行うが、**他ユーザーとの競合解決はしていない**。版管理（`version` 列 + 409）も持たず、同じタスクを同時に編集すると後に保存した方で上書きされ、先に書いた側はリロードするまで気づかない。並べ替えは fractional indexing で「別々のタスクを動かす限り互いを踏まない」ようにして競合を構造的に減らしているが、これは緩和であって解決ではない。きちんとやるなら (1) 版列 + 409 で競合検出、(2) Supabase Realtime で他者の変更をライブ反映、の順で入れる。
- **リアルタイム同期が未実装**。他ユーザーの変更は再読み込みで反映される（MVP としては手動更新の方針）。
