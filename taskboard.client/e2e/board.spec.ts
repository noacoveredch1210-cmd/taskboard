...
test("タスクを別の列へドラッグすると、その列に移り、動いた 1 件だけが保存される", async ({
  ...
  // 連番方式なら後続の行も保存されるが、中間値方式なので動いた 1 件だけ。
  const moves = api.find("POST", "/tasks/");
  expect(moves).toHaveLength(1);
  expect(moves[0].path).toBe(`/tasks/${TASK_A}/move`);
  // order_index は送らない（採番はサーバーの担当）。送るのは移動先と両隣。
...
test("カードのある列の余白へ落とすと、末尾ではなく先頭に入る", async ({
  ...
  // 先頭に入るので「次は B、前は無し」を送る（値ではなく位置を伝える）。
  const moves = api.find("POST", "/tasks/");
  expect(moves).toHaveLength(1);
...
test("3 列のとき、一番右のカードを空の真ん中の列へドラッグできる", async ({
  ...
  const moves = api.find("POST", "/tasks/");
  expect(moves).toHaveLength(1);
  expect(moves[0].body).toMatchObject({ positionId: POS_DOING });
});
...
test("同じ列の中で並べ替えると、動いたタスクだけが保存される", async ({
  ...
  const moves = api.find("POST", "/tasks/");
  expect(moves).toHaveLength(1);
  expect(moves[0].path).toBe(`/tasks/${TASK_B}/move`);
  // 先頭へ移したので「前は無し、次は A」。採番はサーバーが決める。
  expect(moves[0].body).toMatchObject({ prevTaskId: null, nextTaskId: TASK_A });
});

test("保存に失敗したら通知を出し、サーバーの状態へ巻き戻す", async ({ page }) => {
  await signIn(page);
  // 並べ替えの保存だけ 500 にする。巻き戻しのための再取得（GET）は成功させる。
  await stubApi(page, undefined, {
    failWhen: (method, path) => method === "POST" && path.endsWith("/move"),
  });
  await openBoard(page);
